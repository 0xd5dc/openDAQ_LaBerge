...
period = 1  # period = 1ms
numberPoints = 37  # collect 37 data-points
pinput = 4  # A4 as single ended input
ninput = 0  # negative-input=GND
nSamples = 20  # Mean of 20 samples per plotted data-point (10 micro-second period)
gain = GAINx2  # +-2V input maximum corresponding to x2 gain

dq.create_stream(1, period)  # Create the Stream: data channel#, period between data-points in milliseconds
dq.setup_channel(1, numberPoints,
                 1)  # Setup Channel: data channel#, number of data-points to collect, number of repetitions
dq.conf_channel(1, 0, pinput, ninput, gain,
                nSamples)  # Configure Channel: data channel#, analog input mode, pos input, neg input, gain/max input volts, nSamples
